...
public class SimpleChatFrameClient extends JFrame {
	...
	public void sendMessage() {
		...
		// RCAS : si le msg est ".bye", on ferme la fenetre.
		String line = IfClientServerProtocol.BYE;
		String line1[] = line.split(IfClientServerProtocol.SEPARATOR);
	 ...
	 * Create the frame.
	 */
	public SimpleChatFrameClient(IfSenderModel sender,
			ListModel<String> clientListModel, Document documentModel) {
		this.sender = sender;
		...
		JMenuItem mntmEnregistrerSous = new JMenuItem(
				Messages.getString("SimpleChatFrameClient.6")); //$NON-NLS-1$
		mnFile.add(mntmEnregistrerSous);

		JMenu mnOutils = new JMenu(
				Messages.getString("SimpleChatFrameClient.7")); //$NON-NLS-1$
		mnOutils.setMnemonic('O');
		menuBar.add(mnOutils);

		JMenuItem mntmEnvoyer = new JMenuItem(
				Messages.getString("SimpleChatFrameClient.8")); //$NON-NLS-1$
		...
		final JCheckBoxMenuItem chckbxmntmAfficher = new JCheckBoxMenuItem(
				Messages.getString("SimpleChatFrameClient.chckbxmntmAfficher.text")); //$NON-NLS-1$

		mnOutils.add(chckbxmntmAfficher);
		...
		list.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				int iFirstSelectedElement = ((JList) e.getSource())
						.getSelectedIndex();
				if (iFirstSelectedElement >= 0
						&& iFirstSelectedElement < listModel.getSize()) {
		...
		// MSAI on ajoute un onglet qui contiendra le scrollPaneText

		tabbedPane.addTab("Salon général", null, scrollPaneText, null);
		...
		JCheckBoxMenuItem chckbxmntmLock = new JCheckBoxMenuItem(
				Messages.getString("SimpleChatFrameClient.10")); //$NON-NLS-1$
		...
		textField.getActionMap().put(
				Messages.getString("SimpleChatFrameClient.13"), sendAction); //$NON-NLS-1$
		...
		gl_panel.setHorizontalGroup(gl_panel.createParallelGroup(
				...
				gl_panel.createSequentialGroup()
						.addComponent(lblSender, GroupLayout.PREFERRED_SIZE,
								GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(textField, GroupLayout.DEFAULT_SIZE, 267,
								Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(btnSend, GroupLayout.DEFAULT_SIZE, 57,
								Short.MAX_VALUE)));
		...
		// MSAI code pour afficher la toolbar si la case est cochée

		chckbxmntmAfficher.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) {
				if (chckbxmntmAfficher.isSelected()) {
					toolBar.setVisible(true);

				} else if (!chckbxmntmAfficher.isSelected()) {
					toolBar.setVisible(false);
				}
			}
		});
	...
	private class SendAction extends ResourceAction {
		private Icon getIcon() {
			return new ImageIcon(
					SimpleChatFrameClient.class.getResource("send_16_16.jpg")); //$NON-NLS-1$
		}
	...
	private class LockAction extends ResourceAction {
		public LockAction() {
			...
			putValue(SHORT_DESCRIPTION,
					Messages.getString("SimpleChatFrameClient.0")); //$NON-NLS-1$
		}
